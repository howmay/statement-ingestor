...
def build_gmail_query(
    ...
    # 1) Collect all sub-queries
    sub_queries = []
    
    profile_query = _build_statement_profiles_query(statement_profiles or [])
    if profile_query:
        sub_queries.append(profile_query)
        
    if not senders and not keywords:
        # If no explicit filters, always include generic search
        generic_query = _build_generic_statement_query()
        if generic_query not in sub_queries:
            sub_queries.append(generic_query)
    else:
        legacy_query = _build_legacy_query(senders, keywords)
        if legacy_query:
            sub_queries.append(legacy_query)

    if sub_queries:
        query_parts.append(_join_or(sub_queries))
...
def _build_legacy_query(senders: List[str], keywords: List[str]) -> str:
    ...
    query_parts.append("(filename:pdf OR filename:csv)")
