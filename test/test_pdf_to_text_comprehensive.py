...
class TestExtractTextFromPdfBranches:
    ...
    def test_pypdf_importerror_raises(self, tmp_path):
        ...
        with patch("src.parsing.pdf.pdf_to_text._extract_with_pdfium", return_value=""), \
            ...
            # With the new implementation, ImportError is caught and logged
            result = pdfmod.extract_text_from_pdf(str(pdf_file))
            assert result is None  # All extractors failed
