...
def test_result_cache_set_get_and_key(tmp_path):
    with patch.dict(os.environ, {"ENABLE_CONTENT_CACHE": "true"}):
...
def test_result_cache_content_storage_disabled_by_default(tmp_path):
    cache = cache_mod.ResultCache(cache_dir=str(tmp_path))

    key = cache._get_cache_key("abc", "x")
    cache.set("abc", {"transactions": [{"amount": 100}]}, extra="x")

    assert (Path(tmp_path) / f"{key}.json").exists() is False
    assert cache.get("abc", extra="x") is None


def test_result_cache_content_storage_can_be_enabled_via_env(tmp_path, monkeypatch):
    monkeypatch.setenv("ENABLE_CONTENT_CACHE", "true")
    cache = cache_mod.ResultCache(cache_dir=str(tmp_path))
